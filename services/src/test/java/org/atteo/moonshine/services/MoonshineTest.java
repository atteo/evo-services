...
import org.atteo.moonshine.MoonshineFactory;
...
public class MoonshineTest {
	...
	public void shouldStartWithDefaults() throws IncorrectConfigurationException, IOException {
		try (Moonshine moonshine = MoonshineFactory.builder().build()) {
			moonshine.start();
		}
	}
	...
	public void shouldStartWithHomeSet() throws IncorrectConfigurationException, IOException {
		try (Moonshine moonshine = MoonshineFactory.builder()
	...
	@Test
	public void shouldThroughWhenSingletonWithId() throws IncorrectConfigurationException, IOException {
		// given
		try (Moonshine moonshine = MoonshineFactory.builder()
				.homeDirectory("target/test-home/")
				.addConfigurationFromString(""
				...
				+ "</config>")
				.build()) {
			when(moonshine).start();

			// then
			assertThat(caughtException()).isInstanceOf(IncorrectConfigurationException.class)
					.hasMessage("Service 'org.atteo.moonshine.services.SingletonService' is marked as singleton, but has an id specified");
		}
	}
