    ...
    public void start() {
        if (discoverResources) {
            ...
            for (ResourceFactory rf : resourceFactories) {
                resteasyFilter.getDispatcher().getRegistry().addResourceFactory(rf);
            }

            for (com.google.inject.Provider<?> p : restProviders) {
                resteasyFilter.getDispatcher().getProviderFactory().registerProviderInstance(p.get());
            }
        }
    }
}
